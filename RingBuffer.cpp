...
//returns number of elements not pushed
template <typename T>
size_t RingBuffer<T>::push(T *element, size_t count)
{
    size_t writes = min(contigWrite(NULL), count); //how many elements can we write at once?

    while(writes > 0)
    {
        if(element != NULL) memcpy(writeHead, element, writes * sizeof(T));
        ...
        writes = min(contigWrite(NULL), count); //how many elements can we write at once?
    }
    return count;
}
...
//returns number of elements not popped
template <typename T>
size_t RingBuffer<T>::pop(T* element, size_t count)
{
    size_t reads = min(contigRead(NULL), count); //how many elements can we write at once?

    while(reads > 0)
    {
        if(element != NULL) memcpy(element, readHead, reads * sizeof(T));
        advanceRead(reads);     //advance the write pointer
        element += reads;       //advance the read pointer
        count -= reads;         //subtract the written count

        reads = min(contigRead(NULL), count);   //how many elements can we write at once?
    }
    return count;
}
...
template <typename T>
size_t RingBuffer<T>::contigRead(T** pReadHead)
{
    if(pReadHead != NULL) *pReadHead = readHead;
...
template <typename T>
size_t RingBuffer<T>::contigWrite(T** pWriteHead)
{
    if(pWriteHead != NULL) *pWriteHead = writeHead;
